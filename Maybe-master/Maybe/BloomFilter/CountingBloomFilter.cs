...
namespace Maybe.BloomFilter
{
    ...
    {
        private readonly int[] _collectionState;
        public int BrojClanova { get; set; }

        public CountingBloomFilter(int arraySize, int numHashes) : base(arraySize, numHashes)
        {
            _collectionState = new int[arraySize];
            BrojClanova = 0;
        ...
        {
            return index < 0 || index >= _collectionState.Length ? (int)0 : _collectionState[index];
        }

        public string GetPopunjenost()
        {
            string popunjenost = "";
            for (int i = 0; i < _collectionState.Length; i++)
            {
                if (i == _collectionState.Length)
                {
                    popunjenost += this.CounterAt(i);
                    break;
                }
                popunjenost +=this.CounterAt(i)+",";
            }
