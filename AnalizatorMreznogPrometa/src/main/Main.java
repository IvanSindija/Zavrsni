	...
	//			.Load(new String[] { "ACK(SYN)", "SYN-ACK" });
		...
		if(greskeFilteraZaSveK.size() > 0){
			...
			try (Writer writer = new BufferedWriter(
					new OutputStreamWriter(
							new FileOutputStream(
									"C:/faks/zavrsni/rezultati/JavaIntervaliGresakaOdnosSZahtjevimaUBloom.csv"),
							"utf-8"))) {
				int i=0;
				for (LinkedList<Integer> greskeK : greskeFilteraZaSveK) {
					String linija = "";
					...
					for(int j=0;j<intervali.size();j++){
						if(j==0){
							linija+=((double)greskeK.get(intervali.get(j))/intervali.get(j))+",";
						}
						else if(j==intervali.size()-1){
							linija+=((double)(greskeK.get(intervali.get(j))-greskeK.get(intervali.get(j-1)))/(intervali.get(j)-intervali.get(j-1)));
						}else{
							linija+=((double)(greskeK.get(intervali.get(j))-greskeK.get(intervali.get(j-1)))/(intervali.get(j)-intervali.get(j-1)))+",";
						}
					}
